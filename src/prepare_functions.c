...
uint8_t *prepare_settings(uint8_t *idx, uint8_t ***args, uint8_t *key, uint8_t (*fun)(Stepper*, uint8_t*))
{
	...
	if(strcmp((void*)args[0][0], (void*)KEYS.STEPPER) == 0) // checks if the selected string is "spp" (stepper)
	{
		...
		{
			if(strcmp((void*)args[1][0], (void*)key) == 0) // key is SPEED or MICROSTEPPING
			{
				if(!fun(stepper, args[1][1])) // try to setup SPEED or MICROSTEPPING and if failed ..
				{
					if(strcmp((void *)key, (void*)KEYS.SPEED) == 0) // if SPEED
			...
			else // key is not SPEED & MICROSTEPPING 
			{
				if(strcmp((void *)key, (void*)KEYS.SPEED) == 0) // if the key should be spped
					feedback = cmd_builder_buildErr(idx, (uint8_t*)"10"); // "no speed key"
				else // should be microstepping
