...
uint8_t ***connector_parse(uint8_t *dt, uint8_t *args_size)
{
	...
	for (param = strtok((uint8_t*) dt, DIALOG_DELIMITER); param != NULL; param = strtok(NULL, DIALOG_DELIMITER))
	{
		...
		args[*args_size-1] = (uint8_t **) malloc(2 * sizeof(uint8_t *));
        args[*args_size-1][0] = (uint8_t *)malloc(data_size(key) * sizeof(uint8_t));
        args[*args_size-1][1] = (uint8_t *)malloc(data_size(value) * sizeof(uint8_t));

		args[*args_size - 1][0] = key;
