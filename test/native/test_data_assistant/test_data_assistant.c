...
/************************************** DATA_STRING_SIZE TESTS ******************************************************/

// test if data_size return 0 if the string is empty
void test_data_size_should_be_zero() 
{
    uint8_t string[64] = "";
    TEST_ASSERT_EQUAL_UINT8(0, data_size(string));
}

// test if data_size returns 3 if string length is 3
void test_data_size_should_be_three() 
{
    uint8_t string[64] = "123";
    TEST_ASSERT_EQUAL_UINT8(3, data_size(string));
}

// test if data_size returns 5 if string length is 5
void test_data_size_should_be_five() 
{
    uint8_t string[64] = "12345";
    TEST_ASSERT_EQUAL_UINT8(5, data_size(string));
}
...
{
    uint8_t string[64] = "01234567890123456789012345678901";
    data_build(string);
...
int main()
{
    ...
    RUN_TEST(test_data_size_should_be_zero);
    RUN_TEST(test_data_size_should_be_three);
    RUN_TEST(test_data_size_should_be_five);
